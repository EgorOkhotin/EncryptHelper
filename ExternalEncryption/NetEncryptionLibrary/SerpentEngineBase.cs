...
namespace ExternalEncryption.NetEncryptionLibrary
{
    public abstract class SerpentEngineBase
    {
        ...
        public int ProcessBlock(byte[] input, int inOff, byte[] output, int outOff)
        {
            ...
            if (input.Length - inOff < BlockSize) throw new ArgumentException("Input buffer is too short or offset is too big");
            if (output.Length - outOff < BlockSize) throw new ArgumentException("Output buffer is too short or offset is too big");
            if (encrypting)
            {
